...
describe('cabeceras de seguridad (next.config.mjs)', () => {
  ...
  it('no conserva orígenes ni directivas del mapa retirado', async () => {
    const rules = await getRules();
    const csp = rules[0].headers.find((h) => h.key.startsWith('Content-Security-Policy'))!;

    expect(csp.value).not.toContain('worker-src blob:');
    expect(csp.value).not.toContain('child-src blob:');
    expect(csp.value).not.toContain('tiles.openfreemap.org');
    expect(csp.value).not.toContain('maptiler');
    expect(csp.value).toContain("connect-src 'self'");
    expect(csp.value).toContain("img-src 'self' data: blob:");
    // Sin comodines en los orígenes.
